...
# ── pdfplumber：用于提取文本型 PDF 内容 ──────────────────────────────────────
try:
    import pdfplumber  # noqa: PLC0415

    PDFPLUMBER_AVAILABLE = True
except ImportError:
    pdfplumber = None  # 未安装时置为 None，后续通过 PDFPLUMBER_AVAILABLE 标志判断
    PDFPLUMBER_AVAILABLE = False
    print("提示：未检测到 pdfplumber 库，无法处理 PDF 文件。"
          "请通过 pip install pdfplumber 安装。")

# ── jieba 分词（可选依赖，未安装时自动降级） ──────────────────────────────
try:
    import jieba  # noqa: PLC0415
    import jieba.posseg as pseg  # noqa: PLC0415,F401

    JIEBA_AVAILABLE = True
except ImportError:
    jieba = None  # 未安装时置为 None，后续通过 JIEBA_AVAILABLE 标志判断
    pseg = None
...
def is_only_xiao_fei_person(name, content):
    ...
    start = 0
    occurrence_count = 0
    xiaofei_occurrence_count = 0  # noqa: N806
...
def extract_names_method2(content, surnames):
    ...
    # 构造"被?纳入"的宽松匹配（被 可选，且与纳入之间允许空白）
    # 同时覆盖"被列入"说法
    induct_suffix = r'(?:被\s*)?(?:纳入|列入)\s*失信被执行人?名单'

    patterns = [
        # 带职位前缀（支持"及董事"、"公司董事"、"董事"等）
        rf'(?:公司\s*)?(?:及\s*)?(?:董事长?|总经理|监事长?|法定代表人|实际控制人)\s*([\u4e00-\u9fa5]{{2,4}})\s*' + induct_suffix,
        # 无前缀：直接姓名+被纳入
        rf'([\u4e00-\u9fa5]{{2,4}})\s*' + induct_suffix,
        # 原有严格模式（保留兜底）
...
def extract_names_method4(content, surnames):
    ...
    pattern = r'失信被执行人（姓名/名称）[：:]\s*(\S[^，。；;、]{1,3})'
    matches = re.finditer(pattern, content)
    for match in matches:
        name = match.group(1).strip()
        if is_valid_name(name, surnames, content):
...
def extract_names_method8_synonym(content, surnames):
    ...
    # 构造"被纳入/被列入/..."的同义词合并正则
    inducted_pattern = build_synonym_pattern('inducted')

    # 同时构造字段名称类型的同义词合并正则（用于 "字段名：姓名" 格式）
    field_pattern = build_synonym_pattern('field_name')

    # --- 模式A：姓名 + 同义词组（覆盖所有"被纳入/被列入..."说法）---
    position_prefix = r'(?:(?:公司\s*)?(?:董事长|执行董事|独立董事|非执行董事|监事长|总经理|副总经理|总裁|法定代表人|实际控制人|董事|监事|主席)\s*)?'
    pattern_a = position_prefix + r'([\u4e00-\u9fa5]{2,4})\s*' + inducted_pattern
...
def is_valid_name(name, surnames, content):
    ...
    for surname in surnames:
        if name.startswith(surname):
            ...
            if not 1 <= len(given_name_part) <= 2:
                return False
            break
    ...
    # 过滤掉职位名称
    positions = ['董事', '董事长', '高管', '主席', '监事', '经理', '主任', '科长', '处长', '局长', '部长', '总裁',
                 '总监']
...
def extract_text_from_pdf(file_path):
    """
    从 PDF 文件中提取文本内容。
    - 文本型 PDF：通过 pdfplumber 直接提取文字。
    - 扫描件页面（无文字）：跳过并记录，不进行 OCR。
    返回：(全文字符串, 扫描页列表)
    """
    ...
    if scanned_pages:
        print(f"  [提示] 以下页面疑似扫描件，已跳过（不含可提取文字）：第 {scanned_pages} 页")

    full_text = '\n'.join(all_text)
    return full_text, scanned_pages
...
def extract_info_from_file(file_path, surnames):
    ...
    从单个 PDF 文件中提取信息
    """
    content, scanned_pages = extract_text_from_pdf(file_path)

    if not content.strip():
        print(f"  [警告] 文件无可提取文本（可能全为扫描件），跳过。")
        return {
            '证券代码': '', '证券简称': '', '公告编号': '',
            '公告时间': '', '失信被执行人姓名': '', '案号': '', '执行文号': ''
        }
