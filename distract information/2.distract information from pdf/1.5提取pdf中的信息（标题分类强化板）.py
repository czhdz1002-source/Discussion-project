...
"""
失信被执行人信息提取工具 v11 (分类引擎彻底重构版)
修复与优化：
  1. 动作识别弹性化：解决“撤销公司及董事长失信”、“删除错误失信”等被打断的词组无法识别的问题。
  2. 限消隔离器重构：精准剥离“限制高消费”、“限消”，绝不误杀前文的“纳入失信”，解决变“其他”的BUG。
  3. 扩充职务词与容错：新增“财务总监”，兼容错别字“控股股股东”。
  4. 强化隔离判断：维持文件名与正文分别判定的逻辑，防污染。
"""
...
XIAOFEI_KEYWORDS: tuple[str, ...] = ('限制消费', '限消', '消费令', '限消人员', '限制高消费')
CROSS_SECTION_MARKERS: tuple[str, ...] = ('限制消费', '被限制消费', '限消', '限制高消费')

# 为抽取逻辑保留的同义词表（分类逻辑已改为弹性正则，抽取依然用此表精准定位）
SYNONYM_GROUPS: dict[str, list[str]] = {
    'inducted': [
        '被纳入失信被执行人名单', '纳入失信被执行人名单', '被纳入失信被执行名单', '纳入失信被执行名单',
        '被列入失信被执行人名单', '列入失信被执行人名单', '被认定为失信被执行人', '被加入失信被执行人名单',
        '被纳入失信名单', '纳入失信名单', '列入失信名单', '被列入失信名单', '被纳入失信被执行人', '被列入失信被执行人',
        '被纳为失信被执行人', '纳为失信被执行人', '被列入失信联合惩戒对象', '增加失信信息'
    ],
    'field_name': [
...
# ══════════════════════════════════════════════════════════════════════════════
# 标题及分类识别模块 (分类引擎 v11 弹性化重构)
# ══════════════════════════════════════════════════════════════════════════════

def classify_document(filename: str, content: str) -> str:
    # ── 1. 预编译结构化组件库（弹性正则） ──
    # 不再死板匹配全称，只要核心词“不属于/撤销/纳入”与“失信”之间没有断开太远即判定
    not_dish_pat = re.compile(r'(?:不属于|未被|未)(?:纳入|列入|纳为|增加|新增|认定为)?.*?失信', re.UNICODE)
    revoke_pat = re.compile(r'(?:撤销|删除|移出|退出|解除).*?失信', re.UNICODE)
    induct_pat = re.compile(r'(?:纳入|列入|纳为|增加|新增).*?失信', re.UNICODE)

    # 严密职务库：加入财务总监，兼容“控股股股东”，负向断言排雷 董事(?!会)
    roles = r'(董事长|董事(?!会)|监事会主席|监事(?!会)|高级管理人员|高管|总裁|副总裁|总经理|副总经理|财务总监|实际控制人|实控人|控股股?股东|原?股东|法定代表人|法代|董事会秘书|董秘)'
    comps = r'(?:公司|本公司|上市公司|本企业)'
    conns = r'(?:及|与|和|及其|、|，|,)'

    # 严格的双主体匹配逻辑（如：公司及董事长，公司及其控股股东）
    ...
    # ── 2. 精准版：限消隔离器重构 ──
    # 精确匹配并剔除“及相关主体被限制高消费”、“、公司董事长被限消”等分支干扰，绝不删掉主干的失信词
    xiaofei_pat = re.compile(
        r'(?:、|，|,|和|及|并(?:且)?被?|且被?)?\s*(?:公司)?[^、，,。]*?(?:被)?(?:出具|列入)?(?:限制高?消费|限消)[^、，,。]*',
        re.UNICODE
    )
