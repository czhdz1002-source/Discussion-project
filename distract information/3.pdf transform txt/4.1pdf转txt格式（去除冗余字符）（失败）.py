...
class PDFProcessor:
    ...
    def clean_text(self, text):
        ...
        # 去除所有类型的空格（半角空格、全角空格等）
        # 使用正则表达式去除所有空白字符，但保留制表符
        import re
        
        # 匹配所有空白字符，但排除制表符
        # \t是制表符，我们保留它
        # 其他空白字符包括：空格、换页符、垂直制表符等
        text = re.sub(r'[ \u00A0\u2000-\u200F\u2028-\u202F\u205F\u3000]+', '', text)
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if tables:
                        for table_num, table in enumerate(tables, 1):
                            if table:  # 确保表格不为空
                                # 添加表格标记（将在清理时被去除）
                                all_text += f"---第{page_num}页表格{table_num}---"
                                table_count += 1
                                ...
                                for row in table:
                                    ...
                                    # 清理每个单元格中的空格和换行，但保留制表符分隔
                                    cleaned_cells = [self.clean_text(cell) for cell in row_text]
                                    all_text += "\t".join(cleaned_cells)
